...
export class AdminService {
  ...
  async overview() {
    const [totalUsers, paidUsers, freeUsers, totalRevenue, totalRequests, totalTokens] = await Promise.all([
      ...
      this.prisma.user.count({ where: { NOT: { subscriptions: { none: {} } } } }),
      this.prisma.user.count({ where: { subscriptions: { none: {} } } }),
  ...
  async addCredit(userId: string, amount: number, description: string) {
    ...
    await this.prisma.topUpLedger.create({ data: { userId, amount, balanceAfter: prev + amount, source: 'manual_credit', description } });
  ...
  async createModel(dto: any) {
    const model = await this.prisma.model.create({ data: { ...dto, compatibilityModes: Array.isArray(dto.compatibilityModes) ? dto.compatibilityModes.join(',') : dto.compatibilityModes } });
  ...
  async updateModel(id: string, dto: any) {
    if (dto.compatibilityModes && Array.isArray(dto.compatibilityModes)) dto.compatibilityModes = dto.compatibilityModes.join(',');
  ...
  async createPlan(dto: any) {
    const plan = await this.prisma.plan.create({ data: { ...dto, allowedModels: Array.isArray(dto.allowedModels) ? dto.allowedModels.join(',') : dto.allowedModels } });
    return { message: 'Plan created', plan };
  }

  async updatePlan(id: string, dto: any) {
    if (dto.allowedModels && Array.isArray(dto.allowedModels)) dto.allowedModels = dto.allowedModels.join(',');
